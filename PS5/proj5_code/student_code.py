...
def nearest_neighbor_classify(train_image_feats,
    ...
    print(train_labels)
    pred_labels = []
    pair_dists = pairwise_distances(test_image_feats, train_image_feats)
    for i in range(M):
        d = pair_dists[i]
        print(d)
        s = np.argsort(d)
...
def kmeans(feature_vectors, k, max_iter=100):
    ...
    for i in range(max_iter):
        ...
        for j in range(k):
            # print(j)
            cfv = feature_vectors[np.where(c == j)]
            # print(cfv)
            if len(cfv) > 0:
                centroids[j] = np.mean(cfv, axis=0)
        # print(centroids)
...
def build_vocabulary(image_arrays, vocab_size=50, stride=20, max_iter=10):
    ...
    *   The default vocab_size of 50 is sufficient for you to get a
    ...
    for img in image_arrays:
        ...
        v = get_siftnet_features(torch.from_numpy(img).view(1, 1, h, w).type(torch.float32), x, y)
        for fv in v:
    ...
    return kmeans(feature_vectors, k = vocab_size, max_iter=max_iter)
...
def get_bags_of_sifts(image_arrays, vocabulary, stride=5):
    ...
    for i in range(num_images):
        ...
        h, w = img.shape
        x, y = generate_sample_points(h, w, stride)
        fv = get_siftnet_features(torch.from_numpy(img).view(1, 1, h, w).type(torch.float32), x, y)
        c = kmeans_quantize(fv, vocab)
        h, _ = np.histogram(c, bins=np.arange(vocab_size + 1))
        h = h.astype(float)
        h /= np.linalg.norm(h)
        feats[i] = h
