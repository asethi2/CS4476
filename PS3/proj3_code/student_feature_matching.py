...
def compute_feature_distances(features1, features2):
    ...
    for i, f1 in enumerate(features1):
        for j, f2 in enumerate(features2):
            dists[i][j] = np.linalg.norm(f1 - f2)
...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    for i, d in enumerate(d):
        f1 = features1[i]
        sort_d = sorted(enumerate(d), key=lambda t: t[1])
        r = sort_d[0][1] / sort_d[1][1]
        c = 1 / r
        m.append((i, sort_d[0][0], c))

    sort_m = sorted(m, key=lambda t: t[2], reverse=True)
    matches = [[f1, f2] for f1, f2, _ in sort_m]
    confidences = [c for _, _, c in sort_m]
